...
export interface PolicyLoaderOptions extends Partial<HubClientOptions> {
  ...
  /**
   * The delay (in seconds) between successive requests to check for new policy bundles.
   *
   * @remarks
  ...
  /**
   * A callback to invoke when a policy bundle update has failed to load.
   *
   * @defaultValue (no-op)
   */
  onError?: ((error: NotOK) => Awaitable<void>) | undefined;
}
