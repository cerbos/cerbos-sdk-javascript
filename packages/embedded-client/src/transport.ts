...
export class Transport implements _Transport {
  ...
  public async unary<I extends DescMessage, O extends DescMessage>(
    ...
    switch (_methodName(method)) {
      case _methodName(cerbos.method.checkResources):
        return response(
      ...
      case _methodName(cerbos.method.planResources):
        return response(
      ...
      case _methodName(cerbos.method.serverInfo):
        return response(await this.server.serverInfo());

      case _methodName(health.method.check):
        return response(
