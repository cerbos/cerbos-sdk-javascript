<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [@cerbos/core](./core.md) &gt; [auditLogFilterIsSince](./core.auditlogfilterissince.md)

## auditLogFilterIsSince() function

Type guard to check if an [`AuditLogFilter`](./core.auditlogfilter.md) is an [`AuditLogFilterSince`](./core.auditlogfiltersince.md)<!-- -->.

**Signature:**

```typescript
export declare function auditLogFilterIsSince(filter: AuditLogFilter): filter is AuditLogFilterSince;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

filter


</td><td>

[AuditLogFilter](./core.auditlogfilter.md)


</td><td>


</td></tr>
</tbody></table>

**Returns:**

filter is [AuditLogFilterSince](./core.auditlogfiltersince.md)

